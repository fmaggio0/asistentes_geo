...
L.Control.DrawSelector = L.Control.extend({
  onAdd: function(map) {
    ...
    this.edit = L.DomUtil.create(
      'i',
      'fas fa-edit fa-2x pl-2',
      this.drawSelector
    );
    this.drawtools = L.DomUtil.create('div', 'panel-tools', this.drawSelector);
    this.square = L.DomUtil.create(
      'i',
      'fal fa-draw-square fa-2x',
      this.drawtools
    );
    this.circle = L.DomUtil.create(
      'i',
      'fal fa-draw-circle fa-2x',
      this.drawtools
    );
    this.polygon = L.DomUtil.create(
      'i',
      'fal fa-draw-polygon fa-2x',
      this.drawtools
    );
    ...
    L.DomEvent.addListener(this.polygon, 'click', this.drawPolygon, this)
      ...
      function(end) {
        if (end.layer._map) {
          try {
            ...
            var drawGeom = turf.circle(center, radius, {
              steps: 30,
              units: 'meters'
            });
            if (
...
L.control.setEvents = function(lay, context) {
  ...
  var snapMarker = L.marker(map.getCenter(), {
    icon: map.editTools.createVertexIcon({
      className: 'leaflet-div-icon leaflet-drawing-icon leaflet-icon-snap'
    }),
    opacity: 1,
    zIndexOffset: 1000
  });
